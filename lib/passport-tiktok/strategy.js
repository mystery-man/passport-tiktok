...
Strategy.prototype.userProfile = function(accessToken, authData, done) {
    ...
    const fields =
      "open_id,union_id, avatar_url,avatar_url_100, avatar_url_200,avatar_large_url,display_name,bio_description,profile_deep_link,is_verified,follower_count,following_count,likes_count";
    
    this._oauth2._request("GET", url + fields, function (err, body, res) {
      if (err) {
        return done(
      ...
      try {
        ...
        var profile = {
          ...
          profileImage: data.user.avatar_url_100,
          bioDescription: data.user.bio_description,
          profileDeepLink: data.user.profile_deep_link,
